...
import { PingPongLlamaLend, PingPongLlamaLend__factory } from "../../typechain-types";
...
describe("PingPongLlamaLend", function () {
    ...
    let factory: PingPongLlamaLend__factory;
    let pingPong: PingPongLlamaLend;
    ...
    before(async function () {
        ...
        factory = await ethers.getContractFactory("PingPongLlamaLend");
    });
    ...
    it("should reset the rate limit when enough time has passed", async function () {
        ...
        await ethers.provider.send("evm_mine", []);
        await ethers.provider.send("evm_increaseTime", [Math.round(TWO_DAYS)]);
